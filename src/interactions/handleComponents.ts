...
export function handleComponents(
    ...
    let { author_id, action, page, filter } = decodeBuffer(custom_id) as ComponentButtonBuffer
    ...
    if (page && action) {
        ...
        const randomFumoData = makeRandomFumoData(author_id, filter)

        return res.json({
            type: InteractionResponseType.UpdateMessage,
            data: randomFumoData,
        })
    } else interactionsLogger.warn(`Recieved unknown component interaction (custom_id: ${custom_id})`)
}
